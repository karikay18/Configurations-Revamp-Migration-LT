...
func migrateData(db *gorm.DB, batchSize int) {
	...
	if err := tx.Raw("SELECT COUNT(*) FROM `test_environments` WHERE `deleted_at`  IS NULL").Scan(&totalCount).Error; err != nil {
	...
	for {
		...
		if err := tx.Raw("SELECT * FROM `test_environments` WHERE `deleted_at`  IS NULL ORDER BY `id` LIMIT ? OFFSET ?", batchSize, offset).Scan(&testEnvironments).Error; err != nil {
